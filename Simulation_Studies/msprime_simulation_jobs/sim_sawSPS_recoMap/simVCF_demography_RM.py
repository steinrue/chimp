...
def simulateVCFs(out_dir, out_file_tag, replicates, n_samples, rec_rate, mut_rate, genome_length, dem_type, r_seed, treeSeq_switch):
    ...
    for nn in range(replicates):
        ...
        tree_sequence = msprime.simulate( population_configurations= [p0], demographic_events= dem_events, mutation_rate= mut_rate ,  random_seed=np.random.randint(10000000), recombination_map=msprime.RecombinationMap.read_hapmap("chr3map_msp.map"))
