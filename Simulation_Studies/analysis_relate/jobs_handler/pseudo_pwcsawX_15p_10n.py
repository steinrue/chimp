...
if len(sys.argv) == 1:
    ...
    datadir = f"{home_path}/labshare/gupadhya_folder/sims_final/pwcsawX_pseudo/"
    prefix = "15p_10n_Reg_"
    ...
    for i in range(16):
        ...
        i_file.write( "#PBS -l walltime=3:00:00" + "\n" )
...
if len(sys.argv) > 1:
    ...
    binParameters = [1.75205,5.66,.3] # To use relate's default, specify -1 for third index ( [lb,ub,-1] ) then it ignores lb and rb and does default everything
    ...
    ind_labs = [f'tsk_{i*2}-tsk_{i*2 + 1}' for i in range(num_inds)]
    prepareInput (data_folder + input_vcf, input_filtered_vcf, input_haps_file, input_sample_file, num_inds,ind_labs)
