...
public class Function_B3S_LX implements FunctionUnivariate{ 
	...
	Function_B3S_LX(double x_lb, double x_ub, int x_div, double y_c){
		x_low_bound = x_lb;
		x_up_bound = x_ub;
		x_divisions = x_div;
		
		lnx_lb = Math.log(x_low_bound);
		lnx_rb = Math.log(x_up_bound);
	...
	void updateCfromB() {
		...
		for(int i = 0 ; i < c_3splines.getRowDimension() ; i++) {
			...
			// now bs_coefficients should have the appropriatae vector of (c_i, c_i-1, c_i-2, c_i-3)
	...
	void generate_integral_checks(){
		...
		for(int i = 1 ; i < integral_checks.getDimension() ; i++) { // for each knot
			...
			double m1 = x_divisions / (lnx_rb - lnx_lb) ;
			double m0 = m1 * lnx_lb + j ;
			
			RealVector t_poly = new ArrayRealVector(4);
			t_poly.setEntry(0, a);
			t_poly.setEntry(1, b - 3*a*m1);
			t_poly.setEntry(2, (-2*m1*(b - 3*a*m1)) + c  );
			t_poly.setEntry(3, d + 2*m1*m1*(b - 3*a*m1) - c*m1   );
			
			double int_chunk;
			int_chunk = t_poly.dotProduct(new ArrayRealVector(4,1.0));
			int_chunk = int_chunk * Math.exp(1.0 / m1) ;
			int_chunk = int_chunk - t_poly.getEntry(3);
			int_chunk = int_chunk * Math.exp(m0/m1);
	...
	double transform_RX2X(double rawx) {
		...
		double lnx = Math.log(rawx);
		return (x_divisions)*(lnx - lnx_lb)/(lnx_rb - lnx_lb);
		
	}
	
	double transform_X2RX(double cardx) {
		double lnx = lnx_lb + (cardx / x_divisions) * (lnx_rb - lnx_lb);
		return Math.exp(lnx);
	}
	...
	public double integrate0(double raw_x) {
		...
		if(raw_x <= x_low_bound || j < 0) {output = c_3splines.getEntry(3, 3) * raw_x;}
			...
			output = t_poly.dotProduct(new ArrayRealVector(new double[] {Math.pow(xbar, 3),Math.pow(xbar, 2), xbar,1}));
			output = output * Math.exp(xbar / m1) ;
			
			output = output - t_poly.getEntry(3);
			output = output * Math.exp(m0/m1);
	...
	public FunctionUnivariate copy() {
		Function_B3S_LX out = new Function_B3S_LX(x_low_bound, x_up_bound, x_divisions, 1.);
	...
	public void printToFile(String out_file, int density) {
			...
			try {
				...
				scribe.append("x, y \n");
				//scribe.append("B3S_lx params:" + Arrays.toString(getFunctionParameters()) + " : function \n");
	...
	public void printReciprocalToFile(String out_file, int density) {
		...
		try {
			...
			scribe = new FileWriter( out_file+ ".csv");
			scribe.append("x, y \n");
			//scribe.append("B3S_lx params:" + Arrays.toString(getFunctionParameters()) + " : reciprocal function \n");
	...
	public static void main(String[] args) {
		...
		Function_B3S_LX fun = new Function_B3S_LX(1,100,8,1);
		...
		fun.printToFile("/Users/gautam/Desktop/scratch/fun_spline2", 10000);
